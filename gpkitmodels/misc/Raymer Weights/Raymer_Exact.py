...
Ht_Hv = Variable(
    "\\frac{H_t}{h_v}", "-", "0.0 for Conventional Tail; 1.0 for T Tail"
)
...
Ktr = Variable(
    "K_{tr}", "-", "1.18 for Jet with Thrust Reverser; 1.0 Otherwise"
)
Kuht = Variable(
    "K_{uht}",
    "-",
...
Ky = Variable(
    "K_{y}", "ft", "Aircraft Pitching Radius of Gyration (\\approx 0.3 L_{t"
)
Kz = Variable(
...
Rkva = Variable(
    "R_{kva}", "kV*A", "System Electrical Rating (Typically 40-60) "
)
...
W_APU_uninstalled = Variable(
    "W_{APU_{uninstalled}}", "lb", "Uninstalled APU Weight"
)
...
Wuav = Variable(
    "W_{uav}", "-", "Uninstalled Avionics Weight (Typically 800-1400"
)
...
W_horizontal_tail = Variable(
    "W_{horizontal-tail}", "lbf", "Weight of Horizontal Tail"
)
...
constraints = [  # Raymer page 459
    ...
    * t_c_root_vt ** (-0.5),
    Kws
    >= 0.75 * (1 + 2 * lambda_w) / (1 + lambda_w) * Bw * tangent(Lambda_w) / L,
    W_fuselage
    >= 0.3280
    * K_door
    * KLg
    ...
    * (Iy * 10 ** (-6)) ** (0.07),
    ...
    W_instruments
    >= 4.509 * Kr * Ktp * Nc ** (0.541) * Nen * (Lf + Bw) ** (0.5),
